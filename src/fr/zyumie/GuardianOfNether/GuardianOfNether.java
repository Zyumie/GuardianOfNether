...
public class GuardianOfNether {
	...
	public void spawnBoss(Location loc, Player spawnedBy) {
		...
		boss.getAttribute(Attribute.FOLLOW_RANGE).setBaseValue(2);
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				if (tickCounter % (regenInterval / 20) == 0) {
					boss.setHealth(
							Math.min(health + regenAmount, boss.getAttribute(Attribute.MAX_HEALTH).getBaseValue()));
					boss.getWorld().spawnParticle(Particle.HEART, boss.getLocation().add(0, 1, 0), 5);
				}

				engagedPlayers.removeIf(
						p -> !p.isOnline() || p.isDead() || p.getLocation().distanceSquared(spawnPoint) > 20 * 20);
	...
	private void giveBossArmor(WitherSkeleton boss) {
		...
		boss.getEquipment()
				.setLeggings(createEnchantedArmor(Material.DIAMOND_LEGGINGS, "§6Jambières of GuardianNether"));
	...
	private void spawnMinions(Location loc, int count) {
		...
		for (int i = 0; i < count; i++) {
			WitherSkeleton minion = loc.getWorld()
					.spawn(loc.clone().add(Math.random() * 5 - 2.5, 0, Math.random() * 5 - 2.5), WitherSkeleton.class);
			...
			minion.setHealth(50);
			minion.getAttribute(Attribute.ATTACK_DAMAGE).setBaseValue(2);
			minion.getAttribute(Attribute.FOLLOW_RANGE).setBaseValue(5);
			minion.getAttribute(Attribute.MOVEMENT_SPEED).setBaseValue(0.2);
	...
	public void announceDeath(WitherSkeleton boss) {
		...
		Bukkit.broadcastMessage(
				"§c" + bossName + "§f: Je vais faire disparaitre tout ce que vous aimer avant de Mourir !");
